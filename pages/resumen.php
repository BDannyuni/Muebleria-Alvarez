...
$sql = "SELECT id_producto, nombre_prod, precio_prod, categoria_nom, stock_prod, proveedor_nom, color_nom, material_nom, tapiz_nom 
        FROM productos p INNER JOIN categorias ca on ca.id_Categoria=p.categoria_prod INNER JOIN proveedores pro on pro.id_proveedor=p.proveedor INNER JOIN color c on c.id_color=p.id_color INNER JOIN material m on m.id_material=p.id_material INNER JOIN tapiz t on t.id_tapiz=p.id_tapiz
